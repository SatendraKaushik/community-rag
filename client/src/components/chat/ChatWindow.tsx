...
import { Send, Bot, User, MessageSquareDot, Mail } from 'lucide-react';
...
export default function ChatWindow({ botId, sessionId, title, promptStarters = [] }: ChatWindowProps) {
        ...
        <Card className="flex flex-col h-full w-full overflow-hidden border-gray-200 shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-gray-200 bg-white">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="font-semibold text-sm text-gray-900">{title || "Chat Assistant"}</h3>
                    </div>
                </div>
            </div>
                    ...
                    <div className="w-full max-w-2xl">
                        ...
                        >
                            <Input
                                ...
                                className="flex-1 border-gray-300 focus:border-gray-600 h-12 text-base"
                                disabled={loading}
                            />
                            <Button
                                type="submit"
                            ...
                            <div className="flex flex-wrap gap-2 justify-center">
                                {promptStarters.map((starter, idx) => (
                                    <button
                                        ...
                                        className="text-sm px-4 py-2 bg-gray-100 border border-gray-300 rounded-full hover:bg-gray-200 hover:border-gray-400 transition-colors text-gray-700"
                                    >
                                        {starter}
                                    </button>
                                ))}
                            ...
                            {messages.map((msg, i) => (
                                ...
                                >
                                    <div className={`max-w-[80%] p-3 rounded-2xl ${msg.role === 'user'
                                        ...
                                        <p className="text-sm leading-relaxed">{cleanText(msg.content)}</p>
                                    </div>
                                </motion.div>
                            ))}
                            {loading && (
                    ...
                    <div className="p-4 bg-white border-t border-gray-200">
                        ...
                        >
                            ...
                            <Button
                                type="submit"
                                disabled={loading || !input.trim()}
                                className="w-12 h-11 p-0 rounded-xl"
                            >
                                <Send className="w-5 h-5" />
                            </Button>
                        </form>
